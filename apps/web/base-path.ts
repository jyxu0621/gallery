...
export function withBasePath(url: string, basePath = GALLERY_BASE_PATH): string {
  ...
  if (url === normalizedBase.slice(0, -1) || url.startsWith(normalizedBase)) return url
...
export function rewriteManifestUrls<T extends ManifestShape>(
  manifest: T,
  ...
  for (const photo of rewritten.data ?? []) {
    if (photo.thumbnailUrl) photo.thumbnailUrl = withBasePath(photo.thumbnailUrl, basePath)
    if (photo.originalUrl) photo.originalUrl = withBasePath(photo.originalUrl, basePath)
  }
